...
public class Cache<T : DataConvertible where T.Result == T, T : DataRepresentable> {
    ...
    lazy var cachePath : String = {
        ...
        let cachePath = NSString( string: basePath).stringByAppendingPathComponent(self.name) as String!
        return cachePath
    }()
    
    func formatPath(formatName formatName : String) -> String {
        let formatPath = NSString(string:self.cachePath).stringByAppendingPathComponent(formatName)
